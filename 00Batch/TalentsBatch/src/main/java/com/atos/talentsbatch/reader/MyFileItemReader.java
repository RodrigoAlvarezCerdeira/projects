package com.atos.talentsbatch.reader;
...
public class MyFileItemReader<T> extends AbstractItemCountingItemStreamItemReader<T>
    ...
    public MyFileItemReader() {
        setName(ClassUtils.getShortName(MyFileItemReader.class));
    }
    ...
    public void setSHeaders(String headers) {
        this.sHeaders = headers;
    }
    ...
    /**
     * Public setter for the number of lines to skip at the start of a file. Can be used if the file contains a header without useful (column name)
     * information, and without a comment delimiter at the beginning of the lines.
     * 
    ...
    /**
     * Factory for the {@link BufferedReader} that will be used to extract lines from the file. The default is fine for plain text files, but this is
     * a useful strategy for binary files where the standard BufferedReaader from java.io is limiting.
     * 
    ...
    /**
     * Public setter for the recordSeparatorPolicy. Used to determine where the line endings are and do things like continue over a line ending if
    ...
    /**
     * @return string corresponding to logical record according to {@link #setRecordSeparatorPolicy(RecordSeparatorPolicy)} (might span multiple lines
     *         in file).
     */
    @Override
