...
public class MyFileItemWriter<T> extends AbstractItemStreamItemWriter<T> implements ResourceAwareItemWriterItemStream<T>, InitializingBean {

    public static final boolean DEFAULT_TRANSACTIONAL = true;

    protected static final Log logger = LogFactory.getLog(MyFileItemWriter.class);
    ...
    public MyFileItemWriter() {
        this.setExecutionContextName(ClassUtils.getShortName(MyFileItemWriter.class));
    }
    ...
    /**
     * Flag to indicate that changes should be force-synced to disk on flush. Defaults to false, which means that even with a local disk changes could
     * be lost if the OS crashes in between a write and a cache flush. Setting to true may result in slower performance for usage patterns involving
    ...
    /**
     * Writes out a string followed by a "new line", where the format of the new line separator is determined by the underlying operating system. If
     * the input is not a String and a converter is available the converter will be applied and then this method recursively called with the result.
     * If the input is an array or collection each value will be written to a separate line (recursively calling this method for each value). If no
     * converter is supplied the input object's toString method will be used.<br>
     * 
     * @param items list of items to be written to output stream
     * @throws Exception if the transformer or file output fail, WriterNotOpenException if the writer has not been initialized.
     */
    @Override
    ...
    private class OutputState {
        ...
        boolean shouldDeleteIfExists = false;
        ...
        public void write(String line) throws IOException {
            ...
            logger.info("FILE: " + resource.getFilename() + " add : " + line);
        ...
        private void initializeBufferedWriter() throws IOException {
            ...
            if (headerCallback != null && !isExist) {
                try {
